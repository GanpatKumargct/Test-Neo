...
from app.services.ast_parser import parse_selenium_steps
from app.services.testneo_generator import steps_to_testneo
from app.services.zip_service import get_python_files_from_zip
...
def run_bulk_conversion(self, bulk_scan_id: int) -> None:
    """Extract ZIP, parse each .py with AST, convert to TestNeo, persist results."""
    ...
    pairs = get_python_files_from_zip(zip_path)
    if not pairs:
        with session_scope() as session:
            scan = session.get(BulkScan, bulk_scan_id)
            if scan:
                ...
                session.add(
                    ConversionLog(
                        ...
                        level="WARNING",
                        message="No .py files found in ZIP. Add Python test files and try again.",
                    )
                )
        return
    ...
    for rel_path, source in pairs:
        try:
            steps = parse_selenium_steps(source)
    ...
    with session_scope() as session:
        ...
        session.add(
            ConversionLog(
                bulk_scan_id=bulk_scan_id,
                level="INFO",
                message=f"Found {len(pairs)} .py file(s) in ZIP.",
            )
        )
